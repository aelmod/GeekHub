...
import static com.github.aelmod.common.util.ConsoleUtils.Color.*;
...
public class App {
    ...
    public static void main(String[] args) {
        showTypeOfDataSourcePicker();
        showMainMenu();
    }

    private static void showTypeOfDataSourcePicker() {
        System.out.println("Select type of data source: ");
        for (int i = 0; i < Inventorization.INVENTORIZATIONS.length; i++) {
            System.out.println(i + 1 + ": " + Inventorization.INVENTORIZATIONS[i].getSimpleName());
        }
        int selectedOption = getIntFromUser(1, Inventorization.INVENTORIZATIONS.length);

        try {
            inventorization = Inventorization.INVENTORIZATIONS[selectedOption - 1].newInstance();
        } catch (Exception e) {
        }
    }
        ...
        while (true) {
            ...
            switch (getIntFromUser(0, 4)) {
                ...
                case 1:
                    ...
                    inventorization.productCurrentCountUpdate(product);
                    break;
                case 2:
                    ...
                    inventorization.productSingOutUpdate(product);
                    break;
                case 3:
